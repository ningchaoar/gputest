...
from transformers import AutoConfig, AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig, AutoModelForSeq2SeqLM, AutoModel
...
if args.deepspeed:
    ...
    # deepspeed --no_local_rank --include localhost:0,1,2,3 run_hf_inference.py -b 1 -d
    # model = AutoModelForCausalLM.from_pretrained(model_name).eval().half()
    model = AutoModelForSeq2SeqLM.from_pretrained("./facebook_nllb-moe-54b", torch_dtype=torch.float16).eval()
    ...
    if args.data_type == "nf4":
        ...
        # model = AutoModel.from_config(model_config).half().eval()
        # model.save_pretrained("facebook_nllb-moe-54b", safe_serialization=True)
        model = AutoModelForSeq2SeqLM.from_pretrained("./facebook_nllb-moe-54b", device_map="auto", torch_dtype=torch.float16).eval()
